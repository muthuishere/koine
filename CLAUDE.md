...
<ctx-optimize>
  ...
  <store>Pre-built knowledge store at `~/ctxoptimize/koine/` (config in `.ctxoptimize/` here).</store>
  ...
  <no-local-store>Fresh clone with nothing at `~/ctxoptimize/koine/`? Run `ctx-optimize up` —
