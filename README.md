...
1. **object keys are sorted BY CODE POINT** — Clojure map iteration order is
   ...
   produced different bytes on the two hosts (fixed in 0.7.3). koine picks
...
```clojure
;; deps.edn — JVM
net.clojars.muthuishere/koine {:mvn/version "0.9.1"}
```

```clojure
;; build.cljgo — cljgo
(defn build [b]
  (dep b "net.clojars.muthuishere/koine" {:mvn/version "0.9.1"})
...
>
> **Upgrade to `0.9.1`.** Both hosts were affected identically, so the
...
> keep-alive.
>
> **Upgrade to `0.9.1`.** Measured twice, independently: 61.07s → 0.56s here,
...
**The API is unstable at `0.9.1`.** `koine.process`, `koine.route` and
